...
Invoke `mco` from `PATH`.

If there is any doubt about which binary will run, verify with `command -v mco`.
...
Run `mco` from the repository you want analyzed, or pass that repository with
`--repo /abs/path/to/repo`. The location of `SKILL.md` is unrelated to both the
target repository and the `mco` binary.
